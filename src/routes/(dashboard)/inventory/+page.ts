...
export const load: PageLoad = async ({ fetch }) => {
	try {
		...
		// 3. Sisipkan headers ke DUA request (products & categories)
		const [productsRes, categoriesRes] = await Promise.all([
			fetch(`${env.PUBLIC_API_URL}/products?limit=100`, { headers }),
			fetch(`${env.PUBLIC_API_URL}/categories`, { headers })
		]);

		// 4. Jika API produk gagal/unauthorized, kembalikan array kosong agar tidak crash
		if (!productsRes.ok) {
		...
		// 5. PENGAMAN: Berikan nilai default [] jika data undefined
		...
		// Warehouses tetap dari products (looping menggunakan safeData, bukan rawData.data)
